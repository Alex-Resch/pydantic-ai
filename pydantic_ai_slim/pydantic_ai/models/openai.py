...
class OpenAIResponsesModel(Model[AsyncOpenAI]):
    ...
    async def _responses_create(
        ...
        if model_settings.get('openai_include_code_execution_outputs') or model_settings.get(
                ...
                if isinstance(chunk, responses.ResponseCompletedEvent):
                    ...
                    if provider_details:
                        for event in self._parts_manager.handle_text_delta(
                            ...
                            yield event
                        if self._model_settings.get('openai_download_code_execution_files') and annotations:
                            for annotation in annotations:
                                file_id = annotation.get('file_id')
                                container_id = annotation.get('container_id')
                                if not file_id or not container_id:
